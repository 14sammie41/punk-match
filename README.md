...
+ When deciding on reaction times and the time taken for certain things to happen on the page, for example, card flips, I wanted to ensure a psychologically reasonable time frame. Memory games are about ensuring the match itself is registered, and if no match is found, allowing the brain to remember what has been seen to allow for the next match to be easier. For those of us of a younger disposition, it is much easier for us to remember things within a quicker time frame. As we get older, it becomes harder to retain information. I would like to be able to add a function to the game where you input your age before you start, meaning the game would change the amount of time the cards were on the screen depending on what your theoretical mental capacity was. I have not done it on this version of the game because I ran out of time to add the function properly, but I think it would have been a nice addition.

## User interactions

I will have 12 cards on show in the game, each of which has a back, with the same image on, just like a set of playing cards. Then on the other side of the cards, there will be six images, each of which has an exact matching card. Then the player simply clicks to choose a card, and then again for a second card. Once the player has picked two cards initially, the game will work out if they match or not. If they match, they will stay flipped over, so the player knows. If they don't match, they will flip back over to the back side automatically, and this will repeat until all cards are matched. Every time a match is found, the 'pairs' score will go up.

### Testing - Automated vs. Manual

When deciding on testing manually or automatically, I had not realised that I would need to add a section on my reasoning and the advantages and disadvantages of the assessment criteria. I have been consistently manually testing throughout the development of the page by having a live version of the page open on Chrome through VSCode, and each change I have made, I have tested. This means my automated testing through Jest was essentially null and void by the time I came to check back on the assessment criteria.

The advantage for me of having a live version and manually testing as I develop is that I can see what the user would see (pre-deployment obviously), meaning my user interaction testing at the end of the project takes a lot less time. The disadvantage from a long-term or bigger project standpoint is that it takes significantly longer to correct issues once you have already written a full function instead of doing it step by step. If this project had been bigger, I would be much more likely to use automated testing to speed up the overall process.

The advantage of automated testing is always going to be the less physical clicks and checking of the whole page. Alongside the ability to write clean code from the first line to the last line. On my page, the biggest area I would have done automated testing on, in hindsight, is getting the 'flip' function to work. I had to spend a lot of time writing and re-writing the whole function as I could not work out the error. As you can see, this is not a small or simple function, so I realise I could have made my life a lot easier had I gone the automated route.
...
+ [Google Fonts](https://fonts.google.com/selection/embed) has been used to give a consistent font across the site. I have chosen this font as it has a clean look but with a bit of a gothic style to it.
...
+ [HTML and CSS3 for dummies book](https://www.dummies.com/article/technology/programming-web-design/html5/html5-and-css3-all-in-one-for-dummies-cheat-sheet-207788/) has been used as a booster on the Code Institute lesson content to enhance my learning and add some extra tips and tricks.
...
+ Having set up the basic framework for the game and being happy with the initial functionality of it, I found that when reducing screen size down to below 768px (tablet screen size), I was going down to only three cards per row. Initially, I adjusted the media queries to reduce the overall size of the game, but did not factor in the overall size of cards plus margins. I decided that the best way to resolve this was to continue from the above error and use media queries to add to the flex characteristics of the page.
...
+ [Punk Guitar](assets/images/punk-guitar.jpeg) alt, A punk-designed guitar with a festival crowd in the background.
...
Three images as follows of the project in action:

+ [The game screen when the page is opened](assets/images/punk-match-in-action1.png)
+ [The game screen with one pair matched and the score section increased by one](assets/images/punk-match-in-action2.png)
+ [The game help screen showing the instructions for the game and the play button](assets/images/punk-match-in-action3.png)
...
3. Install VSCode on your computer or open the browser version [here](https://vscode.dev/).
4. Click the 'Open Remote Repository' button on the home page to clone and or edit as you wish.

To do the above, you may also need to follow these steps:
...
5. Change the current working directory to wherever you want it to be.
...
+ Problem four was a suggestion to put a function inside a function instead of using an arrow function. I have chosen to leave this as is otherwise, it is more confusing to me having a function within a function.
+ Problem six is a result of having a number of actions within a single function. I have again chosen to leave this function as I had written it as this is easier to read and understand due to the complexity of the function itself.
+ Problems five and problem seven through to thirteen are the same as problems one and two, caused by over 80 characters per line. These are comments for my benefit and a note to the user so again must stay as they are.
...
+ When loading up on a laptop, everything is in the right place, clear and not obstructed within the window. The first thing I noticed was that the font for the 'Scores' title was not the same as the rest of the page. This was easily corrected by adding the `h2` selector to my CSS file.
+ Next, I clicked one card to confirm the flip function was working. Which it was, then onto clicking the second card. Conveniently, the second card I picked was a match. This showed that the `match` function was working. This also showed that the incremental increase in the scores section was working because the score went up by one as I had found a match.
...
+ I next checked that the `Reset Game` button was working. Since it wasn't, I sussed out that the first issue with the button was that it didn't have an `href` attribute, therefore, it was not linking back to the page load. The second issue was that the `href` attribute just reconfirms the page that we are already on. Finally, I worked out that I had put it in a `button` tag when it needs to be in a `a` tag to be able to work.
...
+ Warning found for line 57 relating to <section> missing a heading, suggested to change to a <div> if no heading is required, which it isn't in this case, as it is just to keep the button in the centre of the page. Chosen to keep it as a <section> as there is no need for a header, but it gives me better versatility this way.
